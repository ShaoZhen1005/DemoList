...
/**
 * Created by ShaoZhen-PC on 2017/9/25.
 */

public class Path_Hexagram_View extends View {
    ...
    // 当前的状态(非常重要)
    private SearchView.State mCurrentState = SearchView.State.NONE;
    ...
    public Path_Hexagram_View(Context context) {
        this(context, null);
    }

    public Path_Hexagram_View(Context context, AttributeSet attrs) {
        super(context, attrs);
        initAll();
        initPaint();
        initHandler();
    ...
    public void initAll() {
        ...
        // 进入开始动画
        mCurrentState = SearchView.State.STARTING;
            ...
            public void handleMessage(Message msg) {
                ...
                switch (mCurrentState) {
                    case STARTING:
                        // 从开始动画转换好搜索动画
                        isOver = false;
                        mCurrentState = SearchView.State.SEARCHING;
                    ...
                    case SEARCHING:
                        if (!isOver) {  // 如果搜索未结束 则继续执行搜索动画
                            ...
                            mCurrentState = SearchView.State.ENDING;
                            mEndingAnimator.start();
                        }
                        break;
                    case ENDING:
                        // 从结束动画转变为无状态
                        mCurrentState = SearchView.State.NONE;
                        break;
                }
            }
        };
